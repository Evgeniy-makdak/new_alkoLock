/**
 * Vendored from @maplibre/maplibre-gl-leaflet@0.1.3 (ISC).
 * Patched for MapLibre GL JS + Leaflet: safe _transformGL / teardown / zoom handlers.
 * Resolved via craco webpack alias so CI/Docker need not rely on patch-package alone.
 */
(function (root, factory) {
    ...
    L.MaplibreGL = L.Layer.extend({
        options: {
            ...
            // e.g. 0.1 would be 10% of the map view in each direction
            padding: 0.1,
        ...
        // Только прямое обновление transform (MapLibre GL 4.x): не трогаем
        // _getTransformForUpdate — при drag в minified-сборке даёт «undefined._getTransformForUpdate»
        // и срывает приложение; jumpTo в каждом _update может зациклить события.
