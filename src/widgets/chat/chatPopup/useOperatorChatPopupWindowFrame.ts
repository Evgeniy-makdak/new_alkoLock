...
/**
 * Popup (web): outer = contentAt100% × zoomFactor; overflow только расширяет target, baseline не раздувается.
 */
export function useOperatorChatPopupWindowFrame(): void {
  useEffect(() => {
    ...
    /** Только при смене превью или около 100% zoom — не накапливаем «раздувание» после 150%. */
